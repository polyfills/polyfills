...
var mnfy = require('mnfy').js
var crypto = require('crypto')
var compress = require('mz/zlib').gzip
var agents = require('polyfills-db').agents
...
exports._bundle = function () {
  ...
  // create a bundle
  var agent = agents.parse(useragent)
  var polyfills = agents.filter(this.polyfills, agent)
...
exports._build = function (bundle) {
  ...
  return Promise.all(bundle.polyfills.map(function (polyfill) {
    ...
    return mnfy(js).then(function (_minified) {
      return Promise.all([
        compress(js),
        compress(minified = _minified),
      ])
